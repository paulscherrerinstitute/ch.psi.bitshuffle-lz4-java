...
static void throw_OOM(JNIEnv *env) {
    env->ThrowNew(OutOfMemoryError, "Out of memory");
}
...
(JNIEnv *env, jclass cls, jbyteArray srcArray, jobject srcBuffer, jint srcOff, jbyteArray destArray, jobject destBuffer, jint destOff, jint nElements, jint bytesPerElement, jint blockSize) {
    ...
    if (srcArray != NULL) {
        in = (char*) env->GetPrimitiveArrayCritical(srcArray, 0);
    } else {
        in = (char*) env->GetDirectBufferAddress(srcBuffer);
    }
    ...
    if (destArray != NULL) {
        out = (char*) env->GetPrimitiveArrayCritical(destArray, 0);
    } else {
        out = (char*) env->GetDirectBufferAddress(destBuffer);
    }
    ...
    if (srcArray != NULL) {
        env->ReleasePrimitiveArrayCritical( srcArray, in, 0);
    }
    if (destArray != NULL) {
        env->ReleasePrimitiveArrayCritical(destArray, out, 0);
    }
